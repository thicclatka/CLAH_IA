...
class ISX_analyzer(BC):
    def __init__(
        self, path: str | list, sess2process: list, fps: int, window_size: int = 5
    ) -> None:
    ...
    def static_class_var_init(
        self, folder_path: str | list, sess2process: list, fps: int, window_size: int
    ) -> None:
    ...
    def find_eventRate(self) -> None:
        ...
        self.Epochs = {frz_name: [] for frz_name in self.frz_names}
        self.CtxtByEpoch = {frz_name: [] for frz_name in self.frz_names}
        if not self.NoBorisCheck:
            for start, stop in zip(self.FrzDict["START"], self.FrzDict["STOP"]):
                diff = stop - start
            ...
            for frz in self.frz_names:
                ...
                frame_starts = np.searchsorted(self.FrameTimes, start2use)
                frame_stops = np.searchsorted(self.FrameTimes, stop2use)
                self.Epochs[frz] = list(zip(frame_starts, frame_stops))

                epoch_midpoint = np.sum(
                    start2use <= int(len(self.FrameTimes) / (2 * self.fps))
                )
                epoch_endpoint = np.sum(
                    start2use > int(len(self.FrameTimes) / (2 * self.fps))
                )
                self.CtxtByEpoch[frz].append([self.CtxtOrder[0]] * epoch_midpoint)
                self.CtxtByEpoch[frz].append([self.CtxtOrder[1]] * epoch_endpoint)

            self.CtxtByEpoch = {
                key: np.hstack(self.CtxtByEpoch[key]) for key in self.CtxtByEpoch.keys()
            }

            for ctxt in self.CtxtOrder:
                ctxt_mask = self.CtxtByCFrameTimes == ctxt
                frz_mask = self.FRZ_BY_TIME == 1
                unfrz_mask = self.FRZ_BY_TIME == 0
        ...
        for ftype in [self.FRZ, self.NFRZ]:
            if self.Epochs[ftype]:
                self.eventRate[f"MEAN_{ftype}_CELL_BY_EPOCH"] = _find_meanER_by_cell(
                    data=self.eventRate["RAW"],
                    epochs=self.Epochs[ftype],
                )
            else:
    ...
    def plot_CTemp(self) -> None:
        ...
        if self.FrzDict is not None or not self.NoBorisCheck:
            frz_periods = self.Epochs[self.FRZ]
    ...
    def plot_eventRate(self) -> None:
        ...
        for axes in [axes0, axes1]:
            for ctxt_idx, ctxt in enumerate(self.CtxtOrder):
                ...
                if not self.NoBorisCheck:
                    for st, en in self.Epochs[self.FRZ]:
    ...
    def plot_PCA(self) -> None:
        ...
        for frz_type, ctxt in plot_order:
            ...
            self.fig_tools.scatter_plot(
                ax=axes,
                X=pca2use[self.CtxtByEpoch[frz_type] == ctxt, 0],
                Y=pca2use[self.CtxtByEpoch[frz_type] == ctxt, 1],
