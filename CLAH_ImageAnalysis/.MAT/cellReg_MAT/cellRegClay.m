...
% if nargin == 2
%     toAlign = 1;
%     manAlignStruc = varargin{1}; % for output from manual alignment GUI (clay)
%     x1range = manAlignStruc.x1range;
%     y1range = manAlignStruc.y1range;
%     x2range = manAlignStruc.x2range;
%     y2range = manAlignStruc.y2range;
% 
% 
%     A1 = multSessSegStruc(1).A; d1a1 = multSessSegStruc(1).d1; d2a1 = multSessSegStruc(1).d2;
%     A1=full(A1);
%     A1 = reshape(A1,d1a1,d2a1,size(A1,2));
%     A2 = multSessSegStruc(2).A; d1a2 = multSessSegStruc(2).d1; d2a2 = multSessSegStruc(2).d2;
%     A2=full(A2);
%     A2 = reshape(A2,d1a2,d2a2,size(A2,2));
% 
%     A1 = A1(y1range(1):y1range(2), x1range(1):x1range(2), :);
%     A2 = A2(y2range(1):y2range(2), x2range(1):x2range(2), :);
% 
%     xMin = min([size(A1,2) size(A2,2)]);
%     yMin = min([size(A1,1) size(A2,1)]);
% 
%     A1 = A1(1:yMin, 1:xMin, :);
%     A2 = A2(1:yMin, 1:xMin, :);
% 
%     multSessSegStruc(1).A = reshape(A1,size(A1,1)*size(A1,2),size(A1,3));
%     multSessSegStruc(1).d1 = size(A1,1); multSessSegStruc(1).d2 = size(A1,2);
%     multSessSegStruc(2).A = reshape(A2,size(A2,1)*size(A2,2),size(A2,3));
%     multSessSegStruc(2).d1 = size(A2,1); multSessSegStruc(2).d2 = size(A2,2);
% end
...
results_directory= pwd; % '/home/clay/Documents/analysis';
...
% Loading the data: (compile spatial_footprints from multSessSegStruc)
disp('Stage 1 - Loading sessions')

% get spatial footprints of all NMF segmented neurons
% nUnits x d1 x d2
for n = 1:length(fieldnames(multSessSegStruc))
    A = permute(reshape(full(multSessSegStruc.(sprintf("numSess%d", n - 1)).A_SPATIAL), multSessSegStruc.(sprintf("numSess%d", n - 1)).d1, multSessSegStruc.(sprintf("numSess%d", n - 1)).d2, size(multSessSegStruc.(sprintf("numSess%d", n - 1)).A_SPATIAL,2)), [3 1 2]);
    try
        spatial_footprints{n} = A;
    catch
        spatial_footprints{n} = A;
    end
end

id_finder = strfind(multSessSegStruc.numSess0.segDictName, "_");
file_names = {multSessSegStruc.numSess0.segDictName(id_finder(1) + 1:id_finder(2)-1)};
...
for i=1:length(fieldnames(multSessSegStruc))
    n=[];
...
maximal_distance=12; % cell-pairs that are more than 12 micrometers apart are assumed to be different cells (NOTE 050823 making larger to account for inconsistencies in centroid for profile)
