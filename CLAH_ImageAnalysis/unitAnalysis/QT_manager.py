...
class QT_manager(BC):
    ...
    Attributes:
        ...
        fps (int): Window size (in frames) used for smoothing for the event/peak detection. Default is 15
    ...
    Methods:
        __init__(self, program_name, path=[], sess2process=[], fps=15, sdThresh=3, timeout=3, overwrite=None, rewOmit=False):
            Initializes the QT_manager class with the provided parameters.

        static_class_var_init(self, folder_path, sess2process, fps, sdThresh, timeout, overwrite):
    ...
    def __init__(
        ...
        Parameters:
            ...
            fps (float): Window size (in frames) used for smoothing for the event/peak detection. Default is 15
            sdThresh (float): Threshold multiplier for event/peak detection based on the standard deviation of the signal's derivative. Default is 3
            timeout (int): Minimum distance between detected peaks/events in seconds. Default is 3
