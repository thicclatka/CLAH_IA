...
class pks_utils:
    ...
    Parameters:
        fps (int): Window size for running mean. Defaults to 15.
    ...
    def __init__(
        ...
        Parameters:
            ...
            fps (int, optional): Window size for running mean. Defaults to 15.
    ...
    def find_CaTransients(
        ...
        running_mean = runmean(Ca_arr, self.fps)
        dCa_arr = np.diff(running_mean)
        #  threshold in stdev
        threshold = self.sdThresh * np.nanstd(dCa_arr)
        peaks = local_minima(-dCa_arr, self.timeout * self.fps, -threshold)
        peaks = [int(peak) for peak in peaks]

        for j in range(3):  # Iterative re-baselining
            for i in peaks:
                i = int(i)
                start = max(i - 3 * self.fps, 0)
                end = min(i + 2 * self.fps, len(dCa_arr))
            ...
            threshold = self.sdThresh * np.nanstd(dCa_arr)
            pks2 = local_minima(-dCa_arr, self.timeout * self.fps, -threshold)
            peaks = np.sort(
    ...
    def _peak_processor(
        ...
        for i, pk in enumerate(peaks):
            pk = int(pk)
            try:
                timeoutXfps = self.timeout * self.fps
                timeout_frames = int(round(timeoutXfps))
